  ...
  <div class="d-flex flex-column justify-content-around p-3 wrapper">
      <div class="d-flex justify-content-around container p-3">
        ...
        <div>
          ...
          <li v-for="border in country.borders" v-if="border.length > 0">{{border}}</li>
        </div>
      </div>
...
export default {
  ...
  created(){
        this.$http.get(`https://restcountries.eu/rest/v2/alpha/${this.code}`)
        .then(function(response){
          this.country = response.body
        })
        .then(function(response){
          this.$http.get(`https://restcountries.eu/rest/v2/region/${this.country.region}`)
          .then(function(response){
            let randomCountry;
            for(let i = 0; i < response.body.length; i++){
                randomCountry = response.body[Math.floor(Math.random() * response.body.length)]
             if(randomCountry.name !== this.country.name && !this.recommended.includes(randomCountry)){
